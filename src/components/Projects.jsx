    ...
    {projects.map((data) => (
      ...
      ">
        ...
        <div className="p-6 flex-1 flex flex-col justify-between">
          ...
          <div className="flex flex-wrap">
            ...
            <span key={index} className="inline-block bg-zinc-800/90 text-purple-500 border border-purple-500/30 text-xs font-semibold rounded-full px-2 py-1 mr-2 mb-2 transition-all hover:bg-purple-500/10 hover:shadow-lg">{tagdata}</span>
            )}
          </div>
        </div>
      </SwiperSlide>
      ...
      <div className="hidden lg:block w-[100%]">
        <Swiper
          ...
          className="mySwiper w-full h-[500px] mt-8"
        >
          {projects.map((data, i) => (
            <SwiperSlide
              key={i}
              className="bg-red-300 flex items-center justify-center text-white font-bold text-xl"
            >
              {data.title}
            </SwiperSlide>
          ))}
